...
### Craigslist
JSON endpoint — `https://vancouver.craigslist.org/jsonsearch/apa`. The Phase-1 RSS feed (`?format=rss`) was deprecated some time before 2026-05: direct probes from a residential IP return HTTP 403 with three different User-Agents and with/without the `cl_b` cookie handshake, while `/jsonsearch/apa` and the HTML page from the same IP both return 200. The JSON payload is a `[entries, metadata]` tuple where each entry carries `PostingID`, `PostingURL`, `PostingTitle`, `Latitude`/`Longitude`, structured `bedrooms` + `price`, `PostedDate` (unix seconds), and `ImageThumb`. We do not scrape HTML — listing pages stay user-driven via the source URL link in the UI. CI runs against `tests/fixtures/craigslist/sample_jsonsearch.json`, not the live API. robots.txt only disallows `/reply`, `/fb`, `/suggest`, `/flag`, `/mf`, `/mailflag`, `/eaf` — `/jsonsearch/apa` is permitted.
