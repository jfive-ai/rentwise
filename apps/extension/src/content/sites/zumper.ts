/**
 * Zumper content script.
 *
 * Same pattern as `rentals_ca.ts` / `padmapper.ts`. PadMapper is owned by
 * Zumper and the search/detail URL shapes diverge enough that the URL
 * classifier and id parser are different, but the extraction skeleton
 * is parallel.
 */
...
export const SOURCE = "zumper" as const;
export const SCHEMA_VERSION = "2026-05-07";

export const SELECTORS = {
  searchResultsContainer: '[data-test="listings-list"], [data-test-id="listings-list"], main',
  searchResultsCard: '[data-test="listing-card"], [data-test-id="listing-card"]',
  cardTitle: '[data-test="listing-card-title"], .listing-card__title, h3',
  cardPrice: '[data-test="listing-card-price"], .listing-card__price',
  cardBeds: '[data-test="listing-card-beds"], .listing-card__beds',
  cardLink: 'a',
  cardThumb: 'img',
  detailContainer: '[data-test="listing-detail"], main',
  detailTitle: 'h1',
  detailPrice: '[data-test="listing-price"], .listing__price',
  detailBeds: '[data-test="listing-beds"], .listing__beds',
  detailBaths: '[data-test="listing-baths"], .listing__baths',
  detailSqft: '[data-test="listing-sqft"], .listing__sqft',
  detailNeighborhood: '[data-test="listing-neighborhood"], .listing__neighborhood',
  detailDescription: '[data-test="listing-description"], .listing__description',
  detailPhotos: '[data-test="listing-gallery"] img, .listing__gallery img',
} as const;

const SEARCH_PATH_RE = /^\/apartments-for-rent\/[^/]+/;
const DETAIL_PATH_RE = /^\/(?:apartment|building|listings)\/[^/]+/;
...
export function parseListingId(url: string): string | null {
  try {
    const u = new URL(url);
    const m = u.pathname.match(/\/(?:apartment|building|listings)\/([^/]+)/);
